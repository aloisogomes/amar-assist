...
return [
    ...
    'info' => [
         ...
         */
        'description' => <<<'MD'
API REST do Amar Assist: autenticação, transações financeiras, importação de planilha e dashboard.

Valores são persistidos em **centavos** (inteiro). A data usa `Y-m-d`. O tipo é `income` (receita) ou `expense` (despesa).

Rotas autenticadas usam Laravel Sanctum: envie `Authorization: Bearer {token}` (o token vem do cadastro ou do login).
MD,
    ],
